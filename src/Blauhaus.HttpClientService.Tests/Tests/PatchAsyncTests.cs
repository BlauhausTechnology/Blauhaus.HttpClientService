...
{
    public class PatchAsyncTests : BaseHttpClientServiceTest
    {
        [Test]
        public async Task SHOULD_post_content_to_correct_endpoint()
        {
            ...
            //Act
            await Sut.PatchAsync<TestDto, TestDto>(wrapper, CancellationToken.None);

            //Assert
            MockMessageHandler.VerifyUri("http://baseaddress.com/testroute");
            MockMessageHandler.VerifyContent("hello world");
            MockMessageHandler.VerifyMethod(HttpMethod.Patch);
        }
        ...
        public async Task SHOULD_return_deserialized_dto()
        {
            ...
            //Act
            var result = await Sut.PatchAsync<TestDto, TestDto>(wrapper, CancellationToken.None);
        ...
        public void WHEN_post_fails_with_error_message_SHOULD_throw()
        {
            ...
            Assert.ThrowsAsync<HttpClientServiceException>(async () =>
                await Sut.PatchAsync<TestDto, TestDto>(wrapper, CancellationToken.None), "bad luck");
        }
